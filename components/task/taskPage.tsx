...
import { AllTasks } from "@/lib/types";
import { Calendar, Search } from "lucide-react";
import { useState } from "react";
import { priorityOptions } from "../modals/newTaskModal";
import { ProjectStatusColors } from "@/lib/project/data";
import ViewUniqueTask from "./viewtask";

const tableHeaders = ["Tasks", "Status", "Priority", "Project", "Deadline", "Points"];
const filters = ["Most recent", "Deadline", "Priority"];

export type ViewUniqueTasktype = {
    state: boolean,
    task: AllTasks | null
}

export default function TaskPage({tasks} : {tasks: AllTasks[]}){
    const[viewTask, setViewTask] = useState<ViewUniqueTasktype>({
        state: false,
        task: null,
    });
    ...
    return (
    	<>
            { viewTask && <ViewUniqueTask viewTask={viewTask} setViewTask={setViewTask}  /> }
                        ...
                        <table className="w-full min-w-100">
                            ...
                            { filteredTasks.map((task) => {
                                ...
                                return (
                                    <tr
                                        key={task.id}
                                        onClick={() => setViewTask({state: true, task: task})}
